...
export default class InteractionCreateEvent extends Event<'interactionCreate'> {
  ...
  async execute(interaction: Interaction) {
    ...
    try {
      // --- ⚡ CRITICAL FIX: ZAMAN AŞIMI ENGELLEYİCİ (DEFER) ⚡ ---
      // Discord'un 3 saniye sınırına takılmamak ve "Uygulama yanıt vermedi" hatasını çözmek için
      // botun komutu aldığını ve düşündüğünü Discord sunucularına anında bildiriyoruz.
      if (!interaction.replied && !interaction.deferred) {
        await interaction.deferReply().catch(() => {});
      }

      // Komut ana fonksiyonunu tetikle
